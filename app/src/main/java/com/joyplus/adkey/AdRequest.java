...
public class AdRequest {
	...
	public Uri toUri() {
		...
		b.appendQueryParameter("sv", AdkeyData.getSDKVersion());
		...
		if(mDevice != null && mDevice.GetCUSTOMINFO() != null){
			...
			if(info.GetMAC()==null || "".equals(info.GetMAC())){
				if(MacAddress == null || "".equals(MacAddress)){
					...
					AdkeyData.setmMac(MD5Util.GetMD5Code(MacAddress.toUpperCase()));
				}
			}else{
				b.appendQueryParameter("i", MD5Util.GetMD5Code(info.GetMAC().toUpperCase()));
				AdkeyData.setmMac(MD5Util.GetMD5Code(info.GetMAC().toUpperCase()));
			}
